...
/**
insetion of the node and edge network. The net is allowed to have cycles, which will be resolved in the mst function.

Params:
parent_in int  (in)- parent vertex
child_in  int (in)- child vertex
weight_in double (in)- the edge weight
*/
...
Params:
target node*  (in)- the taget pointer
...
/**
sorting net array, from minimum to maximum. This is O(n log (n)) //Reference: https://www.geeksforgeeks.org/c-qsort-vs-c-sort/
Params:
(none, but by defualt it's always sorting the net in mst class)
*/
void mst::sortnet(){
...
/**
delete an edge between two given vertices. O(n) (less than n log n).

Params:
parent_in int  (in)- parent vertex
child_in  int (in)- child vertex

del bool (out)- an indication if the edge has been deleted. (Assume all inputs are valid.)
...
Params:
v int  (in)- the vertex we are given 

degreev int (out)- degree of the vertex in the network (not mst!!)
*/
...
/**
for check array, call this after sort to find the "ultimate" parent.  O(n), but best case is O(1).

Params:
v a  (in)- the vertex we are given 

findparent int (out)- the "ultimate" parent.
*/
...
Params:
(none, but by default, we will pass the entire network, net[] into it for analysis. There is no output except a modification of the 
class variable, mst_weight.
   ...
   // cout<< "m1=" <<m1<<endl; 
    ...
    for(int i=0; i<net.size(); i++){
            ...
            if(pp==pc) {
                ...
                //cycle is detected, we simply don't account for the edge weight and continue the next loop
            }
