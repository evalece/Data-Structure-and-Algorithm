...
void ugDriver::relex(int in){
   ...
   // cout<<"relex---------------"<<endl;
      ...
      //  cout<<"re1"<<endl;
    for(int i=0; i<modi[in].connected_h.size(); i++){

      //  cout<< "eptr->connected_heapify.size()="<< eptr->connected_heapify.size()<<endl;

        if( modi[in].current_cost+ modi[in].connected_h[i].dis< modi[i].current_cost){
            
            modi[i].current_cost=modi[in].current_cost+ modi[in].connected_h[i].dis; 
            modi[i].previous=in;
            //cout<<"10"<<endl;
        }
...
void ugDriver::print_path(int index1, int index2){

    
    vector<int> pp;

    if(shortest_d( index1,  index2)!=INFINITY){
