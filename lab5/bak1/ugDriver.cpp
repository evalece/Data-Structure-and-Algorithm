...
void ugDriver::relex(int in){
    ...
    if(eptr->connected_heapify[0].dis!=INFINITY){
        //replace the last node with the first node and run heapify
        int index2= eptr->connected_heapify[0].nod1;
        int index1= eptr->connected_heapify[0].nod2;
        visited[index1]=1;
        ///checking if we need to relax and update the node
        if(modi[index2].current_cost>eptr->connected_heapify[0].dis){
            modi[index2].previous=index1;
            modi[index2].current_cost=eptr->connected_heapify[0].dis;

            //////debug
            cout<< "eptr->connected_heapify[0].dis= "<< eptr->connected_heapify[0].dis<<endl;
        }


        //delete root and run heapify
        eptr->connected_heapify[0]=eptr->connected_heapify[eptr->connected_heapify.size()-1];
        eptr->connected_heapify.pop_back();
         

        for(int i=0; i< eptr->connected_heapify.size(); i++){

         //eptr->connected_heapify[i].dis;
        ...
        //call relex again and do the same for the next smallest node
